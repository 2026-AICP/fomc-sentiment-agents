...
def strategy_node(state: State) -> State:
    ...
    # 지수를 지우지 않는 이유: 기사 1~2건인 날은 일별 지수가 크게 튀지만(실측 +7.09),
    # 값을 빼면 시계열에 구멍이 생긴다. 값은 남기고 등급만 낮춘다. 발동 목록(fired)도 기록용으로 남긴다.
    gate_reason = None
    news = state.get("news")
...
def notifier_node(state: State) -> State:
    ...
    def deliver_and_log(dec, label):
        ...
        남긴다. read_sent 가 사유 있는 행을 세지 않으므로 '보냄'으로 세지 않는다(자동
        재발송은 아니다 — 설계 §5-4) — 받은 사람이 없으니 두 통이 갈 일도 없다.
        일부만 실패하면 '보냄'이다.
        """
        ...
        if dec.suppressed == nt.SUP_SEND_FAILED:
            msg = (f"{label} 발송 실패 — 받은 사람 없음 (수신 {n} · 실패 {failed}). "
                   f"자동 재발송 없음 — 키·도메인 확인")
            state["log"].append(f"[notifier] {msg}")
            # GitHub Actions 실행 화면에 경고로 뜨게 한다. 파이프라인 단계가
            # continue-on-error 라 잡은 초록색으로 끝나므로, 이 줄이 없으면 전원 실패가
            # 로그 속에 묻힌다(설계 §5-4). 수만 찍고 주소는 찍지 않는다.
            print(f"::error::알림 {msg}", flush=True)
            return
        if not dec.send:
...
def reporting_node(state: State) -> State:
    ...
    # 자료가 시차를 두고 도착하므로(회의록 3주 후), "언제 무엇을 알았나"를 남겨두지 않으면
    # 나중 정보가 과거 기록에 섞여 설명력·예측력 분석이 실제보다 좋아 보인다.
    # 속보치/확정판(grade_final 등)과 역할이 다르다 — 저쪽은 '최종 상태 2개',
    # 이쪽은 축이 하나씩 도착할 때마다의 '전체 이력'이라 분석에서 as_of 재구성이 된다.
    try:
        ...
        record_meeting(state["date"], statement=st, minutes=mn, presser=pr,
                       combined=combined, grade=sig.get("grade", "—"),
                       fired=sig.get("fired", []),
