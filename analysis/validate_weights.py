...
★순환 우려 (부인하지 않음): 50:50 은 팀이 VIX 상관 3점 비교로 채택한 값이라
  "VIX 로 고른 값을 VIX 인접 지표로 검증"하는 순환이 실제로 있다. 다만 탐색이
  3점뿐이고(소수점 튜닝 없음), 세 값의 폭이 0.04 로 작아 가중치에 민감하지 않으며,
  두 축 상관 +0.19 로 균등이 이론적 정석(1/N)이라 영향은 제한적이다.
  → 이 검정은 "50:50 이 최적"이 아니라 "고정된 50:50 이 각 축보다 나은가"만
  본다. 상세는 docs/weight_validation.md §1.
