...
미래 정보 차단(조교님 질문 4-3: 시계열은 무작위 분할 금지):
...
START = "2000-01-01"
HORIZONS = (1, 3, 5, 10)      # 거래일. 주 기준은 1, 나머지는 강건성 확인용
# 주 기준을 1거래일로 두는 이유 (2026-08-27 변경, 이전 값 5):
#   ① 프로젝트의 기존 분석이 즉각 반응을 본다 — analyze_alignment.REACTION_OFFSET=1,
#      backtest.N_DAYS=2. 적중 기준만 5일이면 같은 프로젝트가 서로 다른 창을 쓰게 된다.
#   ② divergence 는 "뉴스와 시장이 어긋났다 → 곧 정렬된다"는 신호다. 정렬이 빠를수록
#      신호가 정보를 가졌다는 증거가 강하다. 창이 길수록 무관한 사건이 끼어들 시간이
#      생겨, 적중이 신호의 성능인지 시장의 기본 변동성인지 구분이 흐려진다.
#   ※ 창을 늘려도 난이도가 낮아지지는 않는다 — 분위수 기준이라 귀무가설이 기간에
#     관계없이 14.4~14.8% 로 거의 같다. 기간 선택은 난이도가 아니라 "무엇을 주장하는가"다.
PRIMARY = 1
PCT = 90                      # 상위 10% = 90분위
CRISIS_YEARS = (2008, 2020)   # 제외 버전용 (조교님 질문 4-4: 위기 한두 개가 결과를 만드는지)
...
def write_doc(res: dict) -> None:
    ...
    lines = [
        ...
        *[_row(h, res["thresholds_ex_crisis"][str(h)]) for h in HORIZONS],
        ...
        f"  귀무가설 {u:.1%} 와 유의하게(p<0.05) 구분하려면 n=60 이 필요하다. n=20 이면",
