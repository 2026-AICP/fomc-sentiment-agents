...
class Thresholds:
    ...
    # ★2026-09-13 재보정 (docs/theta_recalibration_2026-09.md). 이전 0.10 · 0.05 는
    #   T=3.1 시절 분포의 분위수였고, T=1 전환(8240208)으로 톤 척도가 2.2배 넓어져
    #   회의별 발동률이 45.9%(설계 ~15%)였다. 같은 방법(p85·p25)을 T=1 회의 220건에
    #   다시 적용했다. 이 세트는 대시보드 회의별 신호(export_dashboard)가 매일 전체를
    #   재계산하는 파생값이라, 결정에 따라 **과거 221 회의 전체를 다시 매겼다**
    #   (51건 변경, 전후 대조표 outputs/theta_regrade_2026-09-13.csv).
    #   일별 알림(COMBINED, daily_signals.csv 에 저장)의 소급 금지와는 대상이 다르다.
    theta_shift: float = 0.251  # A: |이번 톤 - 직전 톤| p85 (T=1, 95% CI 0.207~0.301). 이전 0.10
    theta_t: float = 0.114      # B·C: 톤 크기 하한 |톤| p25 (T=1, 95% CI 0.082~0.133). 이전 0.05
