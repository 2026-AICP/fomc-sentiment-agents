...
# ── 라이브 구간의 시작일 ─────────────────────────────────────────────────
# 이 날부터의 라이브 일별만 쓴다. 07-03~07-09 도 CSV 에 있지만 수집을 막 켠
# 구간이라 하루 2~11건(중앙 3건)뿐이고, 같은 기간은 Standard 티어로 다시 전수
# 회수해 백필 쪽에 확보해 뒀다.
#
# ★척도 추정에서도 이 경계를 지켜야 한다 (2026-09-11).
#   그 7일만 std 0.4789 라(7/09 은 기사 3건에 지수 -0.852) 라이브 전체 std 를
#   0.2009 에서 0.2390 으로 밀어올린다. 그 오염된 값으로 z-파라미터를 고르면
#   주별·일별 중 어느 쪽이 맞는지 판단이 뒤집힌다 — 실제로 한 번 뒤집혔다.
#   경위와 수치는 docs/headline_norm_issue.md §8.
#
# 표시부(analysis/export_dashboard.py, deploy)도 같은 값을 쓴다. 두 곳이
# 어긋나면 뉴스 페이지는 빼는데 통합지수는 넣는 상태가 된다 — 9/11 까지가
# 그랬다. 정의를 여기 한 곳에 두고 양쪽이 가져다 쓴다.
