...
- 재정규화 방식(가용 성분끼리 z 평균)은 `headline.py`의 "뉴스 없으면 Fed 단독" 폴백과
  같은 철학이지만, 성분 수가 다른 회의 간 결합값의 분산이 완전히 같지는 않다(축 간
  상관이 0.34~0.67로 1보다 작아, 성분이 늘수록 평균의 분산이 다소 줄어든다). 그래서
  같은 회의의 속보치(1~2축)와 확정판(3축)은 절대적 크기가 살짝 다를 수 있다 — 이는
  `grade`(속보치)와 `grade_final`(확정판)이 서로 다른 신호를 낼 수 있다는 뜻이고,
  그 차이는 "그 사이 새 정보(의사록)가 왔다"뿐 아니라 척도 변화 때문일 수도 있다.
  `grade_final`은 그래서 "속보치의 정정"이 아니라 "3축을 다 반영한 별도의 재평가"로
  해석해야 한다 — 대시보드도 이를 병기가 아니라 별도 값으로 노출한다.
- 확정판은 의사록 도착 시점에 딱 한 번만 계산한다 — 그 이후 데이터 정정(예: 오분석
  재처리)은 이 메커니즘의 범위 밖이다.
- 확정 여부는 "3축이 다 찼는지"가 아니라 "minutes 가 포함됐는지"로 판정한다
  (`agents/graph.py::news_node`). `analysis/axis_status.expected_axes()`는
  2011-04~2018 회의도 전부 기자회견을 기대하지만, 실제로는 분기(SEP) 회의에만
  있었다(2019년부터 매 회의) — "3축 완성"을 기준으로 삼으면 그 구간 회의는 minutes
  가 도착해도 영원히 확정되지 않는 문제가 있어, minutes 포함 여부만으로 판정한다.
