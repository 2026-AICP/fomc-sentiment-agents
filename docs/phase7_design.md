...
> 상태: **완료·검증 (2026-07-03).** 원칙: "함수(도구) 먼저 검증 → 에이전트로 승격".
> 근거: 도구는 Phase 3~6에서 완성(scrape·sentiment·aggregate·signals·backtest·report).

---

## ✅ 구현 결과·검증 (2026-07-03) — 팀원 먼저 읽기

검증된 도구들을 LangGraph로 감싼 **무인 멀티에이전트 파이프라인** 완성.

```
Collector → Fed Analyst → News Analyst → Combine(headline) → Market → Strategy → Reporting
```

- **무인 batch 검증:** 9개 회의 → 9개 정상(실패 0). 등급 🟢5·⚪1·🔴3 (신호가 실제로 구분).
- **두 경로 확인:** 발표일 주변 뉴스 있으면 News+Fed 통합(headline), 없으면(과거 회의) Fed 단독 폴백.
- **신뢰성:** 전 과정 규칙기반(LLM 미사용). 리포트 §4 신호 ↔ 에이전트 등급 일치(반응 규약 통일).
- **리포트:** 인덱스 · 감성분해 · 신호(§4) · News/통합(§5, 95% CI 포함).

**실행 방법:**
```
# 단건:      SENTIMENT_ENGINE=finbert python3 agents/graph.py 2026-06-17
# 무인 다건:  SENTIMENT_ENGINE=finbert python3 agents/graph.py --batch
# 뉴스 수집:  python3 engine/news_scrape.py        # Marketaux, .newsapi_key 필요(각자 발급)
# News 지수:  python3 analysis/news_index_live.py
```

**남은 것(선택·별도):** Phase 8 스케줄러 · 뉴스 창 기본값 정리(현 데모 after=20) ·
통합 z-표준화(GPU 재생성 후 활성) · 웹 대시보드(Streamlit, 규칙기반).
...
| **News Analyst + Combine** | 발표일 주변 뉴스 → News 지수(+CI) → Fed와 통합(headline) | `engine/news_scrape.py`, `analysis/news_index_live.py`, `analysis/headline.py` |
...
class PipelineState(TypedDict):
    ...
    news: dict                # {conf_weighted, ci_lo, ci_hi, n_articles} (발표일 주변 뉴스)
    headline: dict            # {headline, method, w_fed, w_news} (Fed+News 통합)
...
```
1단계: 최소 뼈대 (3노드) Collector→Analyst→Reporting          ✅
2단계: 5에이전트 확장 (+Market, +Strategy)                     ✅ (합격선)
2.5단계: News 축 통합 (News Analyst + Combine/headline)        ✅ (2026-07-03)
   + 신호 반응 규약 통일(리포트↔에이전트), 무인 batch 9/9 검증
3단계: LLM 요약                                               ❌ 미채택 (규칙기반 확정)
4단계: 스케줄러 자동화                                          → Phase 8 (별도)
```
...
| 뉴스 수집 | 포함 / FOMC만 | **포함됨** — Marketaux 실시간(운영)·WSJ(검증). News 지수+CI → Fed와 통합(headline) |
