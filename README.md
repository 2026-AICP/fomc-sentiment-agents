...
FOMC 성명문을 문장 단위로 감성 분석하여 News & Fed Sentiment Index를 산출하고,
...
## 협업
- `main` 직접 push 금지. 파트별 브랜치(`feat/...`) → PR → 리뷰 후 머지.
- 모델 가중치·DB·생성물은 git 에 올리지 않음 (`.gitignore` 참조).
