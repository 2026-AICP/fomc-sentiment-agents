...
#   **적용 범위가 달라 두 구간은 서로 다른 규칙이다.** (2026-09 확인)
#     WSJ 백본   ProQuest 가 기사 **본문**에서 F∧M 매칭 → 재필터 없이 전량 사용
#     라이브·백필 Marketaux 가 **본문**에서 F 매칭 → 아래 is_relevant 가
#                **제목+설명**으로 F∧M 을 다시 요구
#   Marketaux 응답에 본문 필드가 없어(가장 긴 텍스트가 snippet 163자) 제목+설명으로만
#   판정할 수밖에 없다. 그래서 본문에 연준·정책 얘기가 있어도 제목·설명에 그 단어가
#   없으면 탈락한다 — 백필 탈락 279,018건 중 221,607건(79.4%)이 이 경우다.
#   ※ 예전 주석은 여기 "과거·라이브 일관"이라고 적혀 있었으나 사실이 아니다.
#     같은 WSJ 자료에 라이브 규칙을 적용하면 38,869건 중 6,748건(17.4%)만 남고,
#     두 방식으로 만든 월별 지수의 상관은 r=0.43 에 그친다.
